...
export default function HamburgerContent({ uniqueIdentifier, header_link_items, drop_down_items }) {
  ...
  return (
    <div data-lenis-prevent className={`${isOpen ? 'h-screen' : 'h-0'} transition-all bg-cream overflow-hidden`}>
        {isOpen && (
                            ...
                            header_link_items
                                ...
                                return(
                                    <li key={index} className="relative w-full">
                                        <div className="">
                                            <div onClick={() => toggleAccordion(i)} className="flex items-center justify-end pb-2 cursor-pointer">
                                                <RichText 
                                                    text={finalWord}
                                                    className="text-deep-green text-xl font-ambit-semibold relative"
                                                />
                                                <span className={`${accordSymbolClassName} relative bottom-[2px]`} />
                                            </div>
                                            <div className={`z-20 bg-cream flex flex-col items-end border-b border-[#E9E8DB] transition-all overflow-hidden ${heightClassName} pr-6`}>
                                                {drop_down_items
                                                    .filter((data) => {
