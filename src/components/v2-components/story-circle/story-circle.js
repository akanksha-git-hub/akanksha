...
export default function StoryCircle({ index, onClick, image, currentIndex, className }) {
       ...
       <div className='relative rounded-full h-full w-full'>
        <div className='absolute -top-[16px] -left-[30px] flex items-center justify-center h-[150%] w-[150%]'>
            <svg className={`${classes['circle__svg']} h-full w-full`}>
                <circle cx='60' cy='60' r="48" className={`circle__progress circle__progress--fill`} style={{strokeLinecap: 'butt'}}></circle>
            </svg>
        </div>
        <PrismicNextImage 
            field={image}
