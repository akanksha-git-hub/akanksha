...
export default function ResourcesCardItemsContainer({
  children,
  itemKeyFn,
  isFilter,
}) {
  const {
    ...
    state: { initial, final },
    isLoading,
    filterValues: { month: filteredMonth, year: filteredYear },
  ...
  if (!isFilter) {
    ...
    if (filteredMonth && filteredYear) {
      ...
      sortedArray = slice.primary.card_items
        .filter((item) => {
  ...
  return (
    <>
      {isLoading ? (
        <Skeleton count={8} itemClassName="!h-[120px]" />
      ) : (
        shaveSlice.map((item, index) => (
          <li
            key={itemKeyFn(item)}
            className={`pb-6 ${index !== shaveSlice.length - 1 ? "border-b border-[#A3A19A]" : ""}`}
          >
            {children(item)}
          </li>
        ))
