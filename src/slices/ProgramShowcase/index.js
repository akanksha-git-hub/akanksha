...
const ProgramShowcase = ({ slice }) => {
  ...
  if(slice.variation === 'optionB') {
    ...
    const startDate = new Date(slice.primary.program_showcase_content[trackIndex].start_date);
    const endDate = new Date(slice.primary.program_showcase_content[trackIndex].end_date);
    ...
    return (
      ...
      >
        ...
        <div
          className="h-auto lg:h-[32.4rem] flex gap-12 mt-16"
        >
          <ul 
          ...
          >
            ...
            >
              {
                slice.primary.program_showcase_content.sort((a, b) => {
          ...
          >
            {slice.primary.program_showcase_content.map((item) => (
              <SwiperSlide className="!flex flex-col md:flex-row" key={item.name}>
          ...
          {slice.primary.program_showcase_content.map((item) => (
            <SwiperSlide className="!flex flex-col gap-2 md:flex-row" key={item.name}>
              ...
              <div className='flex gap-2 md:gap-0 flex-col md:flex-row'>
                <ImageComponent className='w-full xl:w-[42%]' image={item.image} />
