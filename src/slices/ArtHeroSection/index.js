  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.from(".heading", { opacity: 0, y: 30 })
        .from(".description", { opacity: 0, y: 30 }, "-=0.6")
        .from(".prismic-img", { opacity: 0, scale: 0.95, stagger: 0.2 }, "-=0.6")
        .from(".product-card", { opacity: 0, y: 40, }, "-=0.5");
    }, heroRef);
  ...
  return (
    <section className="w-full" ref={heroRef}>
      ...
      <div className="hidden md:block relative max-w-screen-xl mx-auto mt-8 mb-48">
        ...
        <PrismicImage field={slice.primary.right_image} className="absolute -top-12 right-44 w-48 z-10 prismic-img" />
        <PrismicImage field={slice.primary.top} className="absolute -top-12 right-[40%] w-8 z-10 prismic-img" />
        <PrismicImage field={slice.primary.left_image} className="absolute top-72 left-52 w-8 z-10 prismic-img" />
