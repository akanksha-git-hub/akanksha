...
const TimelineScroller = ({ slice }) => {
  ...
  const handleMobileUpdateActiveIndex = (i) => {
    ...
    if (i !== swipMobRef.current.swiper.activeIndex)
      swipMobRef.current.swiper.slideTo(i);
  };

  const handleUpdateActiveIndex = (i) => {
    // onClick plotPoints
    ...
    setSlideIndex((prevState) => ({
      ...prevState,
      progressBar: i === 0 ? 5 : progressHeight,
      plotPoints: i,
      resetProgress: i,
    }));
    ...
    timeline.current = gsap.timeline({
      scrollTrigger: {
        ...
        onUpdate: (self) => {
          ...
          if (swiperInstance && activeIndex !== swiperRef.current.activeIndex)
            swiperRef.current.swiper.slideTo(activeIndex);

          setSlideIndex((prevState) => ({
            ...prevState,
            progressBar: activeIndex === 0 ? 5 : progressHeight,
            plotPoints: activeIndex,
          }));
        },
      },
    });
    ...
    <>
      ...
      >
        <div className="w-full relative 950px:h-[600px]">
          ...
          <div className="w-1 hidden 1000pixel:block bg-[#D2D1CD] h-[664px] absolute right-12 xl:right-24 3xl:top-0 z-20 rounded-full">
            <div
              className={`w-full rounded-full custom-bezier relative h-full`}
            >
              <p className="text-black text-2xl font-ambit-regular absolute -left-[94%] -translate-x-2/4 -top-10 ">
                1991
              </p>
              <div
              ...
              <p className="text-black text-2xl font-ambit-regular absolute -left-[8%] -translate-x-2/4 -bottom-12">
...
/>
                      ...
                      />
                      {slideIndex.current === i && (
                        <div className="green-gradient absolute bottom-0 left-0 h-full w-full -z-10 opacity-anim" />
                      )}
                    </div>
                  </SwiperSlide>
                );
                    ...
                    if (point.milestone) {
                      return (
                        <li
                          ...
                          className={`rounded-full !h-[10px] !w-[10px] slow-bezier cursor-pointer relative bg-v2-yellow`}
                        >
                          <span
                            className={`
                          absolute bottom-[50%] translate-y-2/4 -left-[150%] translate-x-2/4 h-[200%] w-[200%] 
                          border-[2.4px] border-[#FBDA1D] 
                          rounded-full slow-bezier
                        `}
                          />
                      ...
                      >
                        <span
                          className={`
                              ...
                              border-[1px] ${i <= slideIndex.plotPoints ? "border-[#FBDA1D]" : "border-[#B6B6B6]"} 
                              rounded-full slow-bezier
                            `}
                        />
                      </li>
